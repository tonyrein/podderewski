...
"""
    ...
    its list.
"""
def update(feed_list = None):
    PodService.update_subscribed_feeds(feeds_list)
#     feed_list = []
#     if feeds_to_update:
...
"""
def download(feeds_list = None):
    PodService.download(feed_list, overwrite = False, new_only = True)
...
def set_property(feed_list=None,key=None,value=None):
    ...
    if key == 'keep':
        ...
        set_episodes_keep_count(cls, i, feed_list)
        return RET_SUCCESS
    if key == 'description':
